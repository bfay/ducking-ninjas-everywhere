...
if( $catalyst_multisite )
{
	...
	$catalyst_uploader_settings['uploadpath'] = CHILD_ROOT . '/css/images/' . $catalyst_multisite . '/'; // The full size images will be stored here. Must have forward slash on end.
	$catalyst_uploader_settings['realpath'] = CHILD_URL . '/css/images/' . $catalyst_multisite . '/'; // This is the real URL location of your gallery images, this is used by the admin script to porvide a full URL link to the uploaded images.
	$catalyst_uploader_settings['adminthumbpath'] = CHILD_ROOT . '/css/images/' . $catalyst_multisite . '/adminthumbnails/'; // Regardless of whether or not you have enabled automatic thumbnail creation above, a 100 pixel wide admin thumbnail is always created for use by the admin panel when listing images.
	$catalyst_uploader_settings['adminthumbpath2'] = CHILD_URL . '/css/images/' . $catalyst_multisite . '/adminthumbnails/';
}
else
{
	$catalyst_uploader_settings['uploadpath'] = CHILD_ROOT . '/css/images/'; // The full size images will be stored here.  Must have forward slash on end.
	$catalyst_uploader_settings['realpath'] = CHILD_URL . '/css/images/'; // This is the real URL location of your gallery images, this is used by the admin script to porvide a full URL link to the uploaded images.
	$catalyst_uploader_settings['adminthumbpath'] = CHILD_ROOT . '/css/images/adminthumbnails/'; // Regardless of whether or not you have enabled automatic thumbnail creation above, a 100 pixel wide admin thumbnail is always created for use by the admin panel when listing images.
	$catalyst_uploader_settings['adminthumbpath2'] = CHILD_URL . '/css/images/adminthumbnails/';
}
$catalyst_uploader_settings['filetypes'] = array( "image/gif", "image/pjpeg", "image/jpeg", "image/x-png", "image/png");  // Only these filetypes are allowed to be uploaded.
...
{
	global $catalyst_uploader_settings, $message;
	...
	// Check if the file being uploaded matches the $catalyst_uploader_settings['filetypes'], if so continue, if not return an error.
	elseif( in_array( $_FILES['image']['type'], $catalyst_uploader_settings['filetypes'] ) )
	{
		...
		$filename = $catalyst_uploader_settings['uploadpath'] . catalyst_clean_filename( $_FILES['image']['name'] );
		...
		$adminthumbname = $catalyst_uploader_settings['adminthumbpath'] . catalyst_clean_filename( $_FILES['image']['name'] );
		...
		{
			@chmod( $catalyst_uploader_settings['uploadpath'] . $_FILES['image']['name'], 0644 );
			@chmod( $catalyst_uploader_settings['adminthumbpath'] . $_FILES['image']['name'], 0644 );
		}
	...
	// The below is displayed if the file being uploaded does not match the specified filetypes held in the $catalyst_uploader_settings['filetypes'] array above.	
	else 
	{
...
function listimages()
{
	...
	foreach( $file_array as $file )  
	{
		...
		list( $width, $height ) = getimagesize( $catalyst_uploader_settings['uploadpath'] . $file );
		
		// Get the file size of each image.
		$file_size = returnFilesize( filesize( $catalyst_uploader_settings['uploadpath'] . $file ) );
		...
		$content .= '<div class="imagecontainer"><div class="thumbnaildiv"><img src="' . $catalyst_uploader_settings['adminthumbpath2'] . '' . $file_thumb . '" alt="' . $file . '" class="thumbnail" /></div>';
		...
		$content .= '<p class="imagelink"><a href="' . $catalyst_uploader_settings['realpath'] . '' . $file . '">' . $catalyst_uploader_settings['realpath'] . '' . $file . '</a></p>';
...
function dorename()
{
	...
	if( $_POST['submit'] == 'Rename' )
	{
		...
		else
		{
			...
			rename( $catalyst_uploader_settings['uploadpath'] . $oldname, $catalyst_uploader_settings['uploadpath'] . catalyst_clean_filename( $newname ) );
			if( file_exists( $catalyst_uploader_settings['adminthumbpath'] . $oldname_thumb ) )
			{
				rename( $catalyst_uploader_settings['adminthumbpath'] . $oldname_thumb, $catalyst_uploader_settings['adminthumbpath'] . catalyst_clean_filename( $newname_thumb ) );
			}
			catalyst_child_folders_close_permissions();
		}
	}
...
function deleteimage()
{
	...
	$content .= '<img src="' . $catalyst_uploader_settings['adminthumbpath2'] . '' . $file_thumb . '" class="boxthumbnail" alt="' . $filename . '" /><br />';
...
function dobulkdelete()
{
	...
	foreach( ( array) $bulk_delete as $filename )
	{
		...
		unlink( $catalyst_uploader_settings['uploadpath'] . $filename );
		if( file_exists( $catalyst_uploader_settings['adminthumbpath'] . $file_thumb ) )
		{
			unlink( $catalyst_uploader_settings['adminthumbpath'] . $file_thumb );
		}
	}
	catalyst_child_folders_close_permissions();
}
